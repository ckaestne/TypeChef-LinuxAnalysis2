...
object CleanFileList {
    ...
    def perform(inputFile: java.io.File,
                outputFile: Option[File],
                openFeatureList: Option[java.io.File] = None,
              featureModelFile: Option[java.io.File] = None
              ) {
        val stderr = new PrintWriter(System.err, true)
        val stdout = new PrintWriter(outputFile.map(new FileOutputStream(_)).getOrElse(System.out), true)
//        if (!featureModelFile.isDefined)
//            stderr.println("Warning: No feature model provided.")
//        if (!openFeatureList.isDefined)
//            stderr.println("Warning: No open-feature list provided.")


        val openFeatures = openFeatureList map (Source.fromFile(_).getLines().toSet)
        val fmFactory = FeatureExprFactory.dflt.featureModelFactory
        val featureModel = featureModelFile.map(f => fmFactory.createFromDimacsFile(f.getAbsolutePath)).getOrElse(fmFactory.empty)
            ...
            pcExpr match {
                case parser.Success(cond: FeatureExpr, _) =>
                    ...
                    if (cond.isSatisfiable(featureModel))
                        stdout.println(fullFilename.dropRight(2) + ": " + cond)
                    else
...
object GeneratePCFiles {
    ...
    case class Config(workingDir: java.io.File = new File("."),
